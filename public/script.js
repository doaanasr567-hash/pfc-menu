...
// ================= Floating assistant: mode picker (voice / chat) =================
...
const pickVoiceBtn = document.getElementById('pickVoice');
const pickChatBtn = document.getElementById('pickChat');
...
let assistantMode = null; // 'voice' | 'chat'

assistantFab.addEventListener('click', () => {
  // If a panel/picker is already open, this button now closes it.
  if (!assistantPanel.classList.contains('hidden') || !assistantPicker.classList.contains('hidden')) {
    closeAssistantAll();
    return;
  }
  assistantPicker.classList.remove('hidden');
});

function closeAssistantAll() {
  assistantPicker.classList.add('hidden');
  assistantPanel.classList.add('hidden');
  if (listening && recognition) recognition.stop();
}

closeAssistantBtn.addEventListener('click', closeAssistantAll);

function openAssistant(mode) {
  assistantMode = mode;
  assistantPicker.classList.add('hidden');
  assistantPanel.classList.remove('hidden');
  voiceModeControls.classList.toggle('hidden', mode !== 'voice');
  chatModeControls.classList.toggle('hidden', mode !== 'chat');
  if (mode === 'chat') setTimeout(() => chatInput.focus(), 50);
}

pickVoiceBtn.addEventListener('click', () => openAssistant('voice'));
pickChatBtn.addEventListener('click', () => openAssistant('chat'));

// Chat mode: same intent engine as voice, just typed instead of spoken.
if (chatModeControls) {
  chatModeControls.addEventListener('submit', (e) => {
    e.preventDefault();
